...
export async function recomputeIntake(userId: number, logDate: string, client?: PoolClient): Promise<number> {
  const q = client ? (text: string, params?: any[]) => client.query(text, params) : query;
  const { rows } = await q<{ total: number }>(
    `SELECT COALESCE(SUM(calories)::INTEGER, 0) AS total FROM food_logs WHERE user_id = $1 AND log_date = $2`,
    [userId, logDate],
  );
  const total = Math.round(rows[0]?.total ?? 0);
  const r = client ? (text: string, params?: any[]) => client.query(text, params) : run;
  await r(
...
export const dailyLogService = {
  ...
  async upsert(userId: number, input: UpsertDailyLogInput): Promise<DailyLog> {
    const recommendedWater = input.weight_kg ? Math.round(input.weight_kg * 0.04 * 1000) : undefined;
    ...
    return transaction(async (client) => {
      ...
      return (await getOne<DailyLog>('SELECT * FROM daily_logs WHERE user_id = $1 AND log_date = $2', [userId, input.log_date]))!;
    });
  },
  ...
  async update(id: number, input: UpsertDailyLogInput): Promise<DailyLog> {
    ...
    return transaction(async (client) => {
      const { rows } = await client.query<DailyLog>(
      ...
      if (input.log_date) await recomputeIntake(current.user_id, input.log_date, client);
      return updated;
    });
  },
};
